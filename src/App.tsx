      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden md:ml-64 relative z-10">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-12 pb-28 text-white">
          <div className="max-w-6xl mx-auto">
            ...
            {currentView === View.TRANSACTIONS && <Transactions transactions={transactions} addTransaction={handleAddOrUpdate} updateTransaction={handleAddOrUpdate} deleteTransaction={(id) => setTransactions(prev => prev.filter(t => t.id !== id))} theme={currentTheme} onSync={handleSyncAll} />}
        ...
        {/* BOTONERA RECUPERADA (ORIGINAL PERO CON COLOR ORO ACTIVO) */}
        <nav className="md:hidden fixed bottom-0 left-3 right-3 bg-navy/95 backdrop-blur-xl border border-white/10 px-4 py-1.5 flex justify-between items-center z-50 rounded-t-2xl shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative -translate-y-4">
            <button onClick={() => setView(View.SCANNER)} className="bg-gold-600 p-3.5 rounded-full border-2 border-white/20 text-white shadow-[0_0_20px_rgba(212,175,55,0.4)]">
              <ScanLine size={24} />
            </button>
          </div>
